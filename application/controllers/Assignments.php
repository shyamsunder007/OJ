...
class Assignments extends CI_Controller
{
	...
	// ------------------------------------------------------------------------
/**
	 * Download CSV file of an assignment (or problem) to browser
	 */
	public function csv($assignment_id, $problem_id = NULL)
	{
		// Find csv file
		if ($problem_id === NULL)
			$pattern = rtrim($this->settings_model->get_setting('assignments_root'),'/')."/assignment_{$assignment_id}/*.csv";
		else
			$pattern = rtrim($this->settings_model->get_setting('assignments_root'),'/')."/assignment_{$assignment_id}/p{$problem_id}/*.csv";
		...
		$filename = shj_basename($csv_files[0]);
		force_download($filename, file_get_contents($csv_files[0]), TRUE);
	}
...
$csv_files = glob("$root_path/p{$i}/*.csv");
			if ($csv_files)
			{
				$path = $csv_files[0];
				$this->zip->add_data("p{$i}/".shj_basename($path), file_get_contents($path));
			...
			if ($names === NULL)
				$data['problems'] = array(
					array(
						...
						'memory_limit' => 50000,
						'allowed_languages' => 'C,C++,Python 2,Python 3,Java,R',
						'diff_cmd' => 'diff',
						'diff_arg' => '-bB',
						'is_upload_only' => 0
					)
...
// Upload CSV File of Assignment
		...
		$this->upload->initialize($config);
		$old_csv_files = glob("$assignment_dir/*.csv");
		$csv_uploaded = $this->upload->do_upload("csv");
		if ($_FILES['csv']['error'] === UPLOAD_ERR_NO_FILE)
			$this->messages[] = array(
				'type' => 'notice',
				'text' => "Notice: You did not upload any csv file for assignment. If needed, upload by editing assignment."
			);
		elseif ( ! $csv_uploaded)
			$this->messages[] = array(
				'type' => 'error',
				'text' => "Error: Error uploading csv file of assignment: ".$this->upload->display_errors('', '')
			);
		else
		{
			foreach($old_csv_files as $old_name)
				shell_exec("rm -f $old_name");
			$this->messages[] = array(
				'type' => 'success',
		...
		if ($zip_uploaded) // if zip file is uploaded
		{
			...
			$this->unzip->allow(array('txt', 'cpp', 'html', 'md', 'pdf','csv'));
			...
			if ( $extract_result )
			{
				...
				shell_exec("cd $assignment_dir;"
					...
					." rm -f */desc.html; rm -f */desc.md; rm -f */*.pdf;rm -f */*.csv;");
				if (glob("$tmp_dir/*.pdf"))
					shell_exec("cd $assignment_dir; rm -f *.pdf");
				if (glob("$tmp_dir/*.csv"))
					shell_exec("cd $assignment_dir; rm -f *.csv");
